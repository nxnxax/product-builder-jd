...
========================================
[보고서식 — 최종 출력 형식 재정의. 반드시 준수]
========================================
위의 모든 작성 규칙(customer_name / summary / AI 의견 / region 등)을 그대로 따른다.
단, 최종 출력 JSON 은 기존 7개 키에 더해 title·tag·bullets·ai_opinion 4개 키를 **반드시** 포함한다.
이 4개 중 하나라도 누락하면 실패다. 절대 생략 금지.

추가 키 정의:
- title: 통화 전체를 한 줄로 압축한 헤드라인. 50자 이내. "~논의함/~문의함/~안내함" 보고서식 종결.
- tag: 통화 성격 한 단어. 문의 / 상담 / 분양 / 결제 / 컴플레인 / 정보수집 / 계약 중 가장 가까운 것. 10자 이내.
- bullets: 핵심 사실 배열. **최소 3개, 최대 5개**. 각 항목 한 문장 "~함/~임" 종결. 마지막 항목은 가능하면 다음 행동.
- ai_opinion: 위 'AI 의견' 작성 규칙과 동일한 분석. 단 "AI 의견:" 접두어 없이 본문만.

최종 출력은 아래 11개 키를 모두 가진 JSON 하나다 (형식 예시 — 내용은 실제 통화 기반):
{
  "customer_name": "고객",
  "summary": "...(줄글 요약, 마지막 단락은 'AI 의견: ...')",
  "interest": "...",
  "inquiry": "...",
  "budget_condition": "...",
  "next_action": "...",
  "region": null,
  "title": "분양 문의 대표번호로 전화한 상대방에게 상담사 연결 안내함",
  "tag": "문의",
  "bullets": [
    "상대방이 분양 문의를 위해 전화함",
    "대표번호로 전화가 걸려와 상담사 연결을 안내함",
    "전화를 끊으면 상담사가 다시 전화할 예정임을 전달함"
  ],
  "ai_opinion": "유지 관리 단계의 적극 사용자. 우선 회신 권장."
}
JSON 외 다른 텍스트 출력 금지. 위 11개 키 전부 포함할 것."""
